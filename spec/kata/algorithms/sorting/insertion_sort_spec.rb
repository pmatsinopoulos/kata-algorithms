...
describe Kata::Algorithms::Sorting::InsertionSort do
  subject {
    Kata::Algorithms::Sorting::InsertionSort.new(nil)
  }
  before do
    subject.array_to_sort = array_to_sort
  end
  context 'when nil array of integers' do
    let(:array_to_sort) { nil }
    it 'sorts the array' do
      expect(subject.sort).to be_nil
    end
  end
  ...
  context 'when array of integers with one element' do
    let(:array_to_sort) { [3] }
    it 'sorts the array' do
      expect(subject.sort).to eq(array_to_sort.sort)
    end
  end
